...
```{r use_caret}
tree_fit <- train(Species ~., data = iris_train, method = "rpart",
...
```{r predict_caret}
predict_on_test <- predict(tree_fit, newdata = iris_test)
predict_on_test
```
...
```{r cm_caret}
confusionMatrix(predict_on_test, iris_test$Species)
```
