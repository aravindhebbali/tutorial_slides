...
`xlim()` and `ylim()` take a numeric vector of length 2 as input. The `expand_limits()` function 
takes two numeric vectors (each of length 2), one for each axis. In all of the above functions, 
the first element represents the lower limit and the second element represents the upper limit.
