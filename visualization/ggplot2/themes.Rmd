---
title: "ggplot2: Themes"
output: 
  revealjs::revealjs_presentation:
    transition: slide
    incremental: true
    theme: default
    highlight: haddock
    center: false
    reveal_options:
      slideNumber: true
      previewLinks: true
    fig_height: 5
    fig_width: 6
---

<style type="text/css">
    .reveal p img {
    align: middle;
  }
  .reveal p {
    text-align: left;
    font-size: 20px;
  }
  .reveal body:after {
    content: url(handshake.png);
    position: fixed;
    bottom: 3.5em;
    left: 3.5em;
    box-shadow: 5px 5px 10px #000; 
  }
  .reveal hr {
    border-color: #0077aa;
  }
  .reveal h1, h2, h3, h4, h5, h6 {
    text-transform: capitalize;
    text-align: center;
    color: #0077aa;
    font-family: 'Helvetica Neue';
    font-size: 25px;
  }
  .reveal ul {
    display: block;
  }
  .reveal ul li {
    color: #3c3c3c; 
    /* set color of list item text */
    list-style: none; 
  }
  .reveal p img {
    align: middle;
  }
  .reveal ul li::before {
      color: #006b6e;  
      /* color of bullet or square */
      content: "\2022"; 
      /* Unicode of character to precede the list item */
      font-size: 1em; 
      /* em or %*/ 
      padding-right: 1.1225em; 
      /* use em*/
      position: relative;
      top: 0em; 
      /* use em or % */
  }
  .reveal ul {
    line-height:150%
  }
  .reveal .fragment {
    font-size: 20px;
  }
  .reveal ol {
    display: block;
  }
</style>

## Agenda

<hr>

In this module, we will learn to 

- modify theme components
- use different themes 


## Intro

<hr>

Use `theme()` to modify the appearance of all non data components of the plot.


## Installation

<hr>

```{r show, eval=FALSE}
library(ggplot2)
library(dplyr)
library(tidyr)
```

```{r load, echo=FALSE, eval=TRUE, results='hide', message=FALSE}
library(ggplot2)
library(dplyr)
library(tidyr)
```

## Scatter Plot

<hr>

```{r theme1, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg))
```

## Axis Title

<hr>

```{r theme2, fig.align='center'}
p + theme(axis.title.x = element_text(
  color = "red", size = 10))
```

## Axis Text

<hr>

```{r theme3, fig.align='center'}
p + theme(axis.text.x = element_text(color = "red", size = 10))
```

## Axis Font

<hr>


```{r theme4, fig.align='center'}
# modify both axis text
p + theme(axis.text = element_text(color = "red", size = 10, face = "italic"))
```

## Axis Ticks

<hr>

```{r theme5, fig.align='center'}
p + theme(axis.ticks = element_line(color = 'blue', size = 1.25, linetype = 2), 
  axis.ticks.length = unit(1, "cm"))
```

## Axis Line

<hr>

```{r theme6, fig.align='center'}
p + theme(
  axis.line = element_line(color = 'red', size = 1.5, linetype = 3))
```

## Legend

<hr>

```{r theme7, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg, color = factor(cyl), shape = factor(gear)))
p
```

----

```{r theme8, fig.align='center'}
p + theme(
  legend.background = element_rect(
    fill = 'gray', 
    linetype = 2, 
    color = "red"
  ),
  legend.margin = margin(4, 8, 4, 8),
  legend.spacing = unit(1.5, "cm"),
  legend.key = element_rect(fill = 'red'),
  legend.key.size = unit(0.4, "cm"),
  legend.text = element_text(color = 'green', face = 'italic'),
  legend.title = element_text(color = 'blue', face = 'bold'),
  legend.title.align = 0.1,
  legend.position = "top",
  legend.direction = "horizontal",
  legend.box = "horizontal",
  legend.box.background = element_rect(fill = "yellow"),
  legend.box.spacing = unit(1, "cm"),
  legend.box.margin = margin(4, 8, 4, 8),
  legend.box.just = "left"
)
```

## Panel

<hr>

## Scatter Plot

<hr>

```{r theme9, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg))
p
```

## Panel Background

<hr>

```{r theme10, fig.align='center'}
p + theme(panel.background = element_rect(fill = 'gray'))
```

## Panel Border

<hr>

```{r theme24, fig.align='center'}
p + theme(
  panel.border = element_rect(fill = NA, color = 'red', linetype = 1, size = 2))
```

## Panel Grid

<hr>

```{r theme25, fig.align='center'}
p + theme(panel.grid = element_line(color = 'blue', linetype = 2, size = 0.5))
```

## Scatter Plot

<hr>

```{r theme11, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg))
p
```

## Panel Grid (Major)

<hr>

```{r theme12, fig.align='center'}
p + theme(
  panel.grid.major.x = element_line(color = 'blue', linetype = 2, size = 0.5))
```


## Panel Grid (Minor)

<hr>

```{r theme26, fig.align='center'}
p + theme(
  panel.grid.minor.y = element_line(color = 'red', linetype = 3, size = 0.2))
```


## Plot Area

<hr>

## Scatter Plot

<hr>

```{r theme13, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg)) + 
  ggtitle('Theme Modification')
p
```

## Plot Background

<hr>

```{r theme14, fig.align='center'}
p + theme(
  plot.background = element_rect(color = 'blue', fill = '#4682B4'))
```

## Plot Title

<hr>

```{r theme27, fig.align='center'}
p + theme(plot.title = element_text(color = 'red'))
```

## Plot Margin

<hr>

```{r theme28, fig.align='center'}
p + theme(plot.margin = unit(c(1, 1, 1, 1), "cm"))
```

## Facets

<hr>

## Scatter Plot

<hr>

```{r theme15, fig.align='center'}
p <- ggplot(mtcars) +
  geom_point(aes(disp, mpg)) + 
  facet_grid(~cyl)
p
```

## Strip Background

<hr>

```{r theme16, fig.align='center'}
p + theme(strip.background = element_rect(color = 'red', fill = '#4682B4'))
```

## Strip Text

<hr>

```{r theme29, fig.align='center'}
p + theme(strip.text = element_text(face = 'italic', size = 8, color = 'red'))
```

## Classic Dark on Light

<hr>

```{r theme17, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_bw()
```

## Default Gray

<hr>

```{r theme18, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_gray()
```

## Light

<hr>

```{r theme19, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_light()
```

## Minimal

<hr>

```{r theme20, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_minimal()
```

## Dark

<hr>

```{r theme21, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_dark()
```

## Classic

<hr>

```{r theme22, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_classic()
```

## Void (Empty)

<hr>

```{r theme23, fig.align='center'}
ggplot(mtcars) +
  geom_point(aes(disp, mpg)) +
  theme_void()
```


## Questions

<hr>

## Thank You

<hr>